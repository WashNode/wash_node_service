...
export class DummyEmailGateway implements IEmailGateway {
    private readonly logger = new Logger(DummyEmailGateway.name);
    ...
    async sendEmail(to: string, subject: string, body: string, htmlBody?: string): Promise<{ success: boolean; messageId?: string; error?: string }> {
        try {
            this.logger.log(`[DUMMY EMAIL] Sending email to ${to} with subject: ${subject}`);
            ...
            this.logger.log(`[DUMMY EMAIL] Email sent successfully with ID: ${messageId}`);
            return {
                success: true,
                messageId,
            };
        } catch (error) {
            this.logger.error(`[DUMMY EMAIL] Failed to send email`, error);
            return {
                success: false,
    ...
    async sendBulkEmail(recipients: string[], subject: string, body: string, htmlBody?: string): Promise<{ success: boolean; messageIds?: string[]; error?: string }> {
        try {
            this.logger.log(`[DUMMY EMAIL] Sending bulk email to ${recipients.length} recipients with subject: ${subject}`);
            ...
            this.logger.log(`[DUMMY EMAIL] Bulk emails sent successfully with ${messageIds.length} message IDs`);
            return {
                success: true,
                messageIds,
            };
        } catch (error) {
            this.logger.error(`[DUMMY EMAIL] Failed to send bulk emails`, error);
            return {
                success: false,
                error: error instanceof Error ? error.message : 'Unknown error occurred',
            };
